...
CREATE TABLE IF NOT EXISTS otel_metrics_series (
    SeriesID UInt64 CODEC(ZSTD(1)),

    MetricType        LowCardinality(String), -- 'gauge', 'sum', 'histogram', 'exponential_histogram', 'summary'
...
CREATE TABLE IF NOT EXISTS otel_metrics_gauge_points (
    SeriesID      UInt64        CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS otel_metrics_sum_points (
    SeriesID      UInt64        CODEC(ZSTD(1)),
    StartTimeUnix DateTime64(9) CODEC(Delta(8), ZSTD(1)),
    TimeUnix      DateTime64(9) CODEC(Delta(8), ZSTD(1)),
    Value         Float64       CODEC(ZSTD(1)),
